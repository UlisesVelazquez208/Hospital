    ...
    export default {
        ...
        mounted() {
            ...
            if(theLanguages !== null && theLanguages.length > 0) {
                selectedLang = theLanguages.filter((item) => {
                    if(item.acronym == localStorage.getItem('locale')) {
                        return item;
                    }
                });
            }
            ...
            if(selectedLang.length > 0) {
                i18n.locale = selectedLang[0].acronym;
                document.querySelector('html').setAttribute('dir', selectedLang[0].text_direction);
            }
        },
...
<style lang="scss">
    ...
    .menu-item, .submenu-header-title, .google-analytics{
        font-size: 14px;
    ...
    .btn-success:hover, .btn-success:active {
        color: #fff !important;
        background-color: #0c6aa2 !important;
        border-color: #0b4c73 !important;
